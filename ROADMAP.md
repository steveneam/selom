...
(`uv sync`: scverse + plotly/kaleido); **(C)** ClawBio runnable via `pip install
clawbio` + Miniforge/mamba per-skill `environment.yml` + bioconda; **(D)** bioSkills
long tail via the bioconda CLI toolchain (samtools/STAR/GATK4/Snakemake/Nextflow…)
promoted through the Skill Foundry; **(E)** Community sandbox via Docker + mamba
images (v2, deferred). A+B immediate, C next, D/E incremental.
