...
- **Cloud state — GOOGLE DRIVE IS CONNECTED + LIVE-VERIFIED (2026-07-25).** Connection `c878e8db-…` on `google-drive`; scope granted = `drive.file` only; refresh token present and a forced refresh works; a live `GET /proxy/drive/v3/about` returns the owner's account (200). **Root cause of the earlier failure was OURS, not Google's:** the OAuth app was already published — `deploy/nango/.env` still carried the pre-proxy `NANGO_SERVER_URL=http://localhost:3003`, so authorize succeeded (redirect_uri comes from the forwarded host) and the code **exchange** then failed `redirect_uri_mismatch` *after* consent → zero connections. Fixed to `https://nango.swordfish.cfd` + container recreated; **`deploy/nango/preflight.sh`** (exit-code gated, failure path proven) now catches it. **Dropbox** integration exists but is **not yet connected**. **OneDrive/Microsoft ON HOLD.**
