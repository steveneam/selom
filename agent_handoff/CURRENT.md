...
- **Resume:**
  ```
  # Resume · 2026-06-14 15:37 +10:00 · Selom · Claude (frontend + backend, acting) · model Opus 4.8 (xhigh)
  ...
  NEXT TASK (BE) — `pathway` skill (Reactome, approach A — the LAST P1 skill; scope `docs/p1-skills-scope.md` §P1.2): coloured pathway map from a gene list + fold-change via Reactome ContentService (open licence; gate KEGG). OPEN DESIGN Q: Reactome diagrams aren't natively editable-Plotly → pick the render representation (recommend a node-link of the pathway's reactions/entities coloured by fold-change, consistent with `go_graph`). pytest + golden test; scoped `feat(backend:)` commit. NB pydeseq2 was installed via `uv pip install pydeseq2` (pandas-3 OK, NOT the multi-GB [omics] stack) — a fresh venv needs it re-installed.
  ...
  ENV: FE — `cd app/frontend; npm run dev:mock -- --port 3010` (mock MSW); npm install adds `motion`; KILL the dev server before ending. BE — user py3.12 (uv at C:/Users/seamegdool/.local/bin/uv.exe); `uv run --directory app/backend python -m pytest` (72) / `-m uvicorn`; SELOM_SKILLS_ENGINE stub|real (golden tests pin stub). Bash CANNOT reach \\cmri.com.au UNC — use PowerShell.
  GIT: `main` ahead of `origin/main` by 1 (the ⌘K palette `7285054`, push on owner's word). pytest 72; ruff clean; FE `tsc` clean. Memory: selom-pipeline-flow-animation, selom-icon-library (owner prefers Phosphor Icons for NEW icon work — FE is currently lucide-react; migration is its own task, not started), selom-real-datasets, ask-before-docker-wsl. End clear-safe.
  ```
