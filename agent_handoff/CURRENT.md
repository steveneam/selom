...
- **① MILESTONE REVIEW — RAN 2026-07-25. Branch is NOT mergeable yet.** `review-gauntlet` (30 confirmed, 3 blockers) + `fe-review` (24 confirmed; **31 of 35 user tasks have no affordance**). Durable backlog: **`docs/milestone-review-2026-07-25/findings.md`** (A = gauntlet, B = fe-review, C = the G1 task walkthrough, D = G2 layout predictions that still need a running-app check). **FIXED so far:** the worst blocker — WS3.1 had flipped DE significance selection from adjusted p to RAW p across 5 runners while the axis/table/methods still claimed "adjusted" (2835 vs 1008 "significant" genes on the real EYG_28 limma export, and the guard test had been inverted to bless it) → `b73bd9b`, adjusted-tier-first resolver + honesty following the tier + ratchets restored, verified on real data. **STILL OPEN:** gauntlet blocker A3 (clicking a new annotation deletes it), the HIGH honesty/provenance set (FACS attributes its compute to FlowKit which is deliberately NOT a dependency + claims compensation it may not have applied; `/data/assemble-scrna` records no lineage; GSEA ranking metric can resolve to a gene-id column; hand-typed significance stars export as an unattributed statistical claim), Nango's Elastic-License-2.0 record, and the Pillar-2 annotation layer's affordance debt. **DECISION PENDING (owner):** finish the annotation layer now vs. flag it off and merge the rest after the honesty fixes.
