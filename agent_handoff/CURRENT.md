...
- **Resume:**
  ...
  ROLE: Codex away → Claude owns BOTH lanes; explicit per-lane scoped commits (NEVER git add -A); keep `## Codex` section drop-in-ready. Selom is DESKTOP-ONLY. Model = Opus 4.8 (xhigh), pinned in .claude/settings.json (NOT Fable 5; do not commit settings.json). Push ONLY when owner asks.
  STATE: B0–B3 done. B4 (publish-confidence) DONE — provenance.py (per-figure reproducibility bundle) + methods.py (auto methods-text) + guardrails.py (statistical guardrails: multiple-testing/FDR + best-effort h5ad & bulk-CSV data checks) ride every /run + job result; FE "Publish confidence" panel (Methods · Reproducibility · Quality checks + warn chip). B4's ONLY remainder = Kaleido journal export (Docker → ON-HOLD).
  P1 (DECISIONS #10; scope = docs/p1-skills-scope.md): DONE = enrichment full GO library (7,727 sets; `uv run --with obonet --directory app/backend python scripts/build_gene_sets.py` → gene_sets_go.json + go_graph/go_dag.json, BOTH gitignored, fall back to committed samples) · DE-table-aware enrichment · use-existing-embedding (umap_scrna embedding param plots a stored UMAP/t-SNE) · go_graph skill (enriched GO terms as an editable DAG node-link). volcano+enrichment+go_graph read real DE-table gene cols (GeneID); scRNA skills got normalize=false for pre-normalized inputs. REMAINING P1: pathway (Reactome ContentService live, approach A — last P1 skill) + deg time-course/no-rep.
  BIG UNBLOCK (owner-directed NEXT = option 2): deg raw-count pyDESeq2 + time-course were data-blocked; the ALPK1 mouse P14/P30/P90 set unblocks BOTH (raw integer counts VERIFIED 32,285×77 + 18-contrast DE over P14/P30/P90 × NR/RPE × DA/MG/WT). Downloaded to D:/selom-data/alpk1/: mouse_P14P30P90/ (rawCounts.xlsx + all_genes.xlsx — XLSX), eyg29/ (St7 rawCounts.csv + DEGs_All_merged.csv — CSV, works today), gene_lists/ (CiliaCarta/MitoCarta/RD_GeneList/retinal markers). NB skills read CSV ONLY → for the mouse xlsx either add xlsx ingestion (openpyxl IS in the venv; add pd.read_excel + the dep) or convert; the EYG29 CSV pair works now.
  FIRST: build/validate deg raw-count pyDESeq2 + a time-course mode — start on the EYG29 CSV pair (no xlsx), then the mouse data; THEN build pathway (approach A). Real data: rpgrip1 ingested (D:/selom-data/rpgrip1/processed/rpgrip1_merged.h5ad, scRNA, dogfooded); EYG_28 staged (bulk DE-results); full catalog docs/real-datasets.md.
  ON-HOLD (Docker/WSL — ASK owner first; agent_handoff/on-hold/): Kaleido export, arq Redis status store, OmicVerse isolated worker, deploy image, community sandbox.
  ENV: backend user py3.12 (uv at C:/Users/seamegdool/.local/bin/uv.exe); `uv run --directory app/backend python -m pytest` (55 passing) / `-m uvicorn`; SELOM_SKILLS_ENGINE stub|real (golden tests pin stub). The Bash tool CANNOT reach \\cmri.com.au UNC shares — use the PowerShell tool for UNC + `& uv.exe ... python` for peeks (xlsx needs openpyxl, in venv). FE dev `npm run dev:mock -- --port 3010`. Gitignored+regenerable: demo.h5ad (scripts/make_demo.py), gene_sets_go.json + go_dag.json (scripts/build_gene_sets.py). pytest 55; ruff clean.
  GIT: main ↔ origin/main in sync @ bfb1a20 (advances with this CURRENT.md commit; push on owner's word). Memory: selom-real-datasets, selom-rpgrip1-real-data, selom-eyg28-bulk-data, ask-before-docker-wsl, selom-competitor-omicsbox (+ index). End clear-safe.
  ```
