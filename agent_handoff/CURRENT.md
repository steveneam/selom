...
- **Resume:**
  ```
  # Resume · 2026-06-14 19:02 +10:00 · Selom · Claude (frontend + backend, acting) · model Opus 4.8 (xhigh)
  ...
  DONE LAST SESSION — GENE-SET BUILDER PHASE A SHIPPED (both lanes, owner-approved): BE `c7ce724` = `gene_sets/library.py` unified loader/search + `GET /gene-sets[/{id}]` + WikiPathways CC0 ingest (`scripts/build_wikipathways.py`, committed `gene_sets/corpus/*.json`, 921 sets/0 unmapped) + apply (enrichment source-select / volcano highlight). FE `c5f9fbe` = `/gene-sets` surface + first-class `GeneSet` in ProjectStore + intent-bus apply with param prefill + Save. pytest 87; ruff/tsc/build clean; browser-verified. NB the GO library (`skills/enrichment/gene_sets_go.json`) is gitignored/regenerable (sample fallback); WikiPathways + curated corpus ARE committed. pydeseq2 still needs `uv pip install pydeseq2` on a fresh venv.
  PHASE: P1 COMPLETE; **gene-set builder Phase A SHIPPED** (DECISIONS #11). build-charter **B4 active** (only Docker-gated Kaleido export remains — ASK owner before any Docker/WSL). NEXT (owner picks): (1) **gene-set builder Phase B** — the keyword→compile→union/intersect builder over the open-core adapters + HGNC dedup/normalization (`docs/gene-set-builder-design.md` §4 Phase B). **PRESENT the Phase-B plan before building (charter rule).** (2) **P2 single-cell depth** (markers/annotate/trajectory — DECISIONS #10). (3) **Licensing follow-up:** confirm provenance of the Fidelle ciliopathy/proteostasis `gene_lists` before shipping them as owned panels (the `curated` source slot is ready; §8.1). (4) real-data dogfooding backlog (`deg` on RPGR + ALPK1 d207, `volcano` on mouse proteomics + RO time-course, `enrichment`/`pathway` vs staged oracles). (5) FE follow-ups: DESIGN.md refresh, Phosphor-icon migration.
  ...
  ENV: FE — `cd app/frontend; npm run dev:mock -- --port 3010` (mock MSW; /gene-sets has an offline fixture); KILL the dev server before ending. BE — user py3.12 (uv at C:/Users/seamegdool/.local/bin/uv.exe); `uv run --directory app/backend python -m pytest` (87) / `-m uvicorn`; SELOM_SKILLS_ENGINE stub|real (golden tests pin stub; `pathway` real engine needs network for Reactome; the WikiPathways/curated corpus loads offline). Real CMRI data + staged copies live under `D:\selom-data` (outside the repo); Bash CANNOT reach \\cmri.com.au UNC — use PowerShell. To regenerate WikiPathways: `uv run --directory app/backend python scripts/build_wikipathways.py` (network).
  GIT: `main` ahead of `origin/main` by 8, ALL UNPUSHED (push on owner's word). Newest: `c5f9fbe` feat(frontend gene-sets) + `c7ce724` feat(backend gene-sets) atop the prior 6. pytest 87; ruff clean; FE tsc + next build clean. Memory: selom-gene-set-builder (Phase A shipped), selom-real-datasets (Fidelle lists licensing TBC), selom-pipeline-flow-animation, selom-icon-library, ask-before-docker-wsl, graphify-keep-current. End clear-safe.
  ```
