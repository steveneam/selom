...
- **Cloud integration (TOP — last step):** when swordfish drops `SELOM_NANGO_SECRET_KEY` → create the two integrations against `nango.swordfish.cfd` (server API, Bearer): **google-drive** (scope `drive.file` → no CASA) + **dropbox** (`files.content.write/read files.metadata.read account_info.read`), client id/secret from `.env`. **Owner's paste (can do NOW, independent):** `https://nango.swordfish.cfd/oauth/callback` into the Google Cloud OAuth client + the Dropbox app redirect URIs. Then owner does one OAuth consent per provider → I push the ERG export to his Drive. **OneDrive/Microsoft ON HOLD** (owner — Azure login trouble).
- **ERG Fig-1E delivery (via Nango, owner's choice — NOT rclone/export):** files at `/home/deploy/migration/selom-migration-staging/selom-data/erg-fig1e/export/n5_sem_2026-07-23/` (PDF/PNG/TIFF + xlsx + 4 CSVs) + the `n5_sem_2026-07-23.zip` (6 MB). Push once the Google Drive integration + owner consent land.
- **ERG figure/table wiring:** surface OP/PhNR/flicker metrics in the figures + `companions/methods.py` — now **UNBLOCKED** (the FACS lane that also touched `methods.py` has landed, `e90203a`).
