...
- **Resume:**
  ```
  # Resume · 2026-06-14 · Selom · Claude (frontend + backend, acting) · model Opus 4.8 (xhigh)
  Selom build repo D:/selom. CLAUDE.md auto-loads. Read agent_handoff/README.md + CURRENT.md (this) + DECISIONS.md + RISKS.md + agent_handoff/on-hold/README.md + docs/build-charter.md + docs/p1-skills-scope.md + docs/real-datasets.md + docs/competitors/omicsbox.md.
  ROLE: Codex away → Claude owns BOTH lanes; explicit per-lane scoped commits (NEVER git add -A); keep `## Codex` section drop-in-ready. Selom is DESKTOP-ONLY. Model = Opus 4.8 (xhigh), pinned in .claude/settings.json (NOT Fable 5; settings.json IS now committed per owner 2026-06-14). Push ONLY when owner asks.
  STATE: B0–B4 done. P1 (DECISIONS #10; scope docs/p1-skills-scope.md) DONE = enrichment full GO library (7,727 sets; `uv run --with obonet --directory app/backend python scripts/build_gene_sets.py` → gene_sets_go.json + go_dag.json, gitignored, fall back to committed samples) · DE-table-aware enrichment · use-existing-embedding · go_graph skill · **deg raw-count pyDESeq2 + time-course + design-sheet input (NEW 2026-06-14, `8700937`)**. REMAINING P1 = pathway (Reactome ContentService, approach A — the last P1 skill) + deg no-replicate (NOISeq-style, no fitting dataset yet).
  deg DETAIL: bulk = pyDESeq2 Wald; group inference strips the trailing replicate suffix (`ctrl_1`→`ctrl`), `reference`/`treatment` pick the contrast (error lists groups if ambiguous), low-count filter + ≥2-rep guard. timecourse (`mode=timecourse`) = time as a continuous covariate, Wald on the time coeff (0.5.4 has NO LRT — use a numpy unit-contrast vector over the design matrix), optional `covariate_col` + `group_col`/`group_val` stratum. Design sheet = optional `design` upload on POST /run, joined on sample id, threaded as reserved `_design_path` (filtered from provenance via resolved_params dropping `_`-keys). xlsx reads via openpyxl (now a CORE dep; re-locked). pydeseq2 was NOT installed → `uv pip install pydeseq2` (pandas-3 OK; NOT the multi-GB [omics] stack).
  REAL DATA: rpgrip1 h5ad (scRNA, dogfooded); EYG_28 bulk DE-results (volcano/enrichment). deg validated on EYG29 CSV (AK1-vs-SCR: TTR/KLK4) + ALPK1 mouse xlsx (contrast NR_P90 Homo-vs-WT via design sheet; time-course 39 NR samples, Genotype-adj, 10,458 genes FDR<0.05 over P14/P30/P90). Mouse design sheet (was missing — cols are opaque S1…S80) = `D:/selom-data/alpk1/mouse_P14P30P90/sample_design_RUV_K2_variates.csv` (Genotype/Age/Tissue/Strain). Catalog docs/real-datasets.md.
  FE FOLLOW-UP (not built): a `design`-sheet upload affordance for bulk/time-course deg — BE accepts it; FE still matrix-only (name inference covers well-named bulk like EYG29).
  ON-HOLD (Docker/WSL — ASK owner first; agent_handoff/on-hold/): Kaleido export (B4's only remainder), arq Redis status store, OmicVerse isolated worker, deploy image, community sandbox.
  ENV: backend user py3.12 (uv at C:/Users/seamegdool/.local/bin/uv.exe); `uv run --directory app/backend python -m pytest` (72 passing) / `-m uvicorn`; SELOM_SKILLS_ENGINE stub|real (golden tests pin stub). Bash CANNOT reach \\cmri.com.au UNC — use PowerShell for UNC + `& uv.exe ... python`. FE dev `npm run dev:mock -- --port 3010`. Gitignored+regenerable: demo.h5ad, gene_sets_go.json + go_dag.json. pytest 72; ruff clean.
  GIT: main ahead of origin/main (NOT pushed): deg `8700937` + docs + this CURRENT.md (+ settings.json). Push on owner's word. Memory: selom-real-datasets (updated — design-sheet recovery), selom-rpgrip1-real-data, selom-eyg28-bulk-data, ask-before-docker-wsl, selom-competitor-omicsbox (+ index). End clear-safe.
  ```
