...
- **Resume:**
  ```
  # Resume · 2026-06-14 16:24 +10:00 · Selom · Claude (frontend + backend, acting) · model Opus 4.8 (xhigh)
  Selom build repo D:/selom. CLAUDE.md auto-loads. Read agent_handoff/README.md + CURRENT.md (this) + DECISIONS.md + RISKS.md + agent_handoff/on-hold/README.md + docs/build-charter.md + docs/p1-skills-scope.md.
  ...
  DONE LAST SESSION — P1 COMPLETE: `pathway` (`1a9604c`, the LAST P1 skill — live Reactome AnalysisService ORA → fold-change-coloured event-hierarchy node-link; stdlib urllib, no new dep; dep-free stub for the golden test; real engine errors cleanly offline) + biomaRt gene-col fix (`6ec924f` — DE skills prefer clean symbol cols over a composite `GeneID`; pathway 0→16 nodes on the real ALPK1 RO data) + Fidelle dogfood data ingested (`95b366d` + `D:\selom-data` — ALPK1 RO/iRPE + RPGR raw counts + GSEA/GO/Reactome oracles + mouse proteomics; `deg` dogfooded on the FIRST human iRPE raw counts). NB pydeseq2 was installed via `uv pip install pydeseq2` (pandas-3 OK, NOT the multi-GB [omics] stack) — a fresh venv needs it re-installed.
  PHASE: skill-breadth **P1 COMPLETE → P2 next** (single-cell depth: markers/annotate/trajectory — DECISIONS #10); build-charter **B4 active** (publish-confidence; only the Docker-gated Kaleido export remains — ASK owner before any Docker/WSL). NEXT (priority order): (1) **APPROVED — gene-set builder Phase A** (DECISIONS #11; `docs/gene-set-builder-design.md` §8): a searchable "Gene Sets" catalog from owned/open sets + a **WikiPathways (CC0) ingest** + the first-class `GeneSet` object + apply-flow; **list-first UX, open-core, MSigDB/KEGG validation-only, ship the Fidelle ciliopathy/proteostasis lists as our own**. Present the Phase-A plan before building (charter rule). (2) my own backlog — real-data dogfooding: `deg` on RPGR + ALPK1 d207, `volcano` on the mouse proteomics + RO DE time-course, `enrichment`/`pathway` vs the staged GSEA/GO oracles (numeric cross-check). (3) FE follow-ups: DESIGN.md refresh, Phosphor-icon migration. `deg` no-replicate (NOISeq) still has no fitting dataset.
  ...
  ENV: FE — `cd app/frontend; npm run dev:mock -- --port 3010` (mock MSW); npm install adds `motion`; KILL the dev server before ending. BE — user py3.12 (uv at C:/Users/seamegdool/.local/bin/uv.exe); `uv run --directory app/backend python -m pytest` (76) / `-m uvicorn`; SELOM_SKILLS_ENGINE stub|real (golden tests pin stub; `pathway` real engine needs network for Reactome). Real CMRI data + staged copies live under `D:\selom-data` (outside the repo); Bash CANNOT reach \\cmri.com.au UNC — use PowerShell.
  GIT: `main` ahead of `origin/main` by 3 (`1a9604c` feat(backend pathway) + `6ec924f` fix(backend gene-col) + `95b366d` docs(data); push on owner's word). pytest 76; ruff clean; FE `tsc` clean. Memory: selom-pipeline-flow-animation, selom-icon-library (Phosphor for NEW icon work — FE is lucide-react; migration not started), selom-real-datasets, ask-before-docker-wsl, graphify-keep-current. End clear-safe.
  ```
