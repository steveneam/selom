...
- **Resume:**
  ...
  NEXT (own pick / owner direction): (1) **NEW FEATURE the owner asked to design 2026-06-14** — a gene-set/list builder ("user types what they want → Selom finds/compiles available license-clean lists → ready to apply to their data"); plan = `docs/gene-set-builder-design.md` (write/refine before building). (2) more real-data dogfooding — `deg` on RPGR + ALPK1 d207, `volcano` on the mouse proteomics + RO DE time-course, `enrichment`/`pathway` vs the staged GSEA/GO oracles (numeric cross-check). (3) FE follow-ups: DESIGN.md refresh, Phosphor-icon migration. `deg` no-replicate (NOISeq) still has no fitting dataset.
