...
import { DEFAULT_INSPECTOR_TAB, inspectorTabs } from "./inspector-tabs";
...
export function PropertyPanel({
  ...
  /** Controlled active tab. Supplied by the inspector dock (`W-2`) so its COLLAPSED spine can expand
   *  straight to a chosen tab; omitted, the panel keeps its own state exactly as before. */
  tab?: string;
  ...
  const tabs = useMemo(() => inspectorTabs(spec), [spec]);
  const hasMarks = tabs.some((t) => t.value === "marks");

  const [uncontrolledTab, setUncontrolledTab] = useState(DEFAULT_INSPECTOR_TAB);
