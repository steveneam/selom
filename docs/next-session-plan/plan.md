...
| ID | Finding | Work | Status |
|---|---|---|---|
| L2-01 | A20 | `GET /cloud/providers` from `cloud/registry.py` + settings; the FE consumes it, static list survives only as an offline-dev fallback. Kills the FE↔BE fork. | **DONE** — route built **through** `cloud.contract.providers_payload` (the dormant route-conformance test in the freeze self-activated: 10 → 11 passing, asserting the route's own bytes). FE: `fetchCloudProviders` maps the wire at the boundary and preserves server menu order; `lib/cloud/providers.ts` is now types + a conservative offline fallback the menu *labels as degraded*; `comingSoon` deleted. Two more client-side truth tables went with it — `lib/cloud/nango.ts`'s `CLOUD_CONNECT_ENABLED` kill-switch (file deleted; the `@nangohq/frontend` seam returns with L2-07) and the hardcoded "Soon" badge. Added `GET /cloud/connections` (+ `registry.provider_for_config_key`) because a Nango connection id is a minted UUID: without it the UI would have to ask a human to paste one, which is another way of saying unreachable. MSW mock mirrors the freeze in the same commit; `/cloud/providers` waiver deleted from the reachability guard. |
| L2-02 | — | Enable `SELOM_CLOUD_GOOGLE` + `SELOM_CLOUD_DROPBOX` in `app/backend/.env` (gitignored). Without this the live OAuth is still refused — one of the three closed gates. | **DONE — and the row named the wrong file.** `config.py` loads **`<repo root>/.env`**, never `app/backend/.env`. So the live Nango URL + secret key staged there were invisible to the server: `nango_base_url` fell back to `http://localhost:3003`, the DEAD dev instance, while `preflight.sh` (which read the same unused file) went green against the live broker. Setting the flags where the row said would have flipped this row DONE with the gate still shut. Fixed by consolidating `SELOM_NANGO_*` + `SELOM_CLOUD_*` into the repo-root `.env` (gitignored — the lead must repeat it on main, exact lines in `LANE-WRAP.md`), pointing `preflight.sh` at that same file, documenting both in `.env.example` + `deploy/nango/README.md`, and locking the two homes together with `tests/test_cloud_env_home.py` (verified to FAIL when they diverge). `GET /cloud/providers` now reports `google`/`dropbox` `enabled: true`, `onedrive` `false`. |
| L2-03 | B14 · B16 | Stop dropping backend-stamped `datasets.source` in the FE dataset mapper — a cloud-imported dataset currently looks hand-dropped. | **DONE** — `fromApiDataset` now maps `source` (snake → camel) through a defensive reader: `datasets.source` is a free-form JSON column, so anything without a `provider` is discarded rather than rendered as a half-empty chip. New `DatasetSource` type; server-authoritative, so `mergeDatasets` needs no change (it only re-attaches client-only fields). Visible in two places in the Data panel — an "Imported · Google Drive" chip on the dataset row (full ref in `title`) and a provenance block at the top of the active card. `lib/projects/sync.test.ts` covers present/absent/malformed. |
| L2-04 | B15 | Kill the fabricated `File` stand-in that becomes `lastFile` and can be POSTed as the run's actual data. Import by reference. [[mock-fallback-never-fabricates-data]] | **DONE** — both fabrications deleted (`new File(["remote"], …)` on cloud import, `new File(["mock"], …)` on re-open); `AnalyzeArgs.file` / `Active.file` are now `File \| null` and a cloud import passes `null` — by reference, the bytes are already in the object store so the run goes through run-from-dataset_id. Also collapsed `Active.real` into file-presence: two fields that must agree is exactly how a fabricated File came to be marked not-real and POSTed anyway. `analyzeExisting` keeps this session's bytes when you re-click the dataset you just dropped. Type change needed no edit in `project-workspace.tsx` (`lastFile` was already `File \| null`). Ratcheted by `lib/cloud/import-surface.guard.test.ts` (the panel constructs no `File` at all). |
| L2-05 | A27 | A user-reachable surface for `/data/assemble-scrna` — it works and nobody can reach it. | **DONE** — the surface is the multi-file drop that already existed, now routed by the drop's own shape: a per-sample scRNA deposit (10x triplet members, or every file a `.h5ad`) goes to `/data/assemble-scrna`; everything else keeps going to the ERG `/data/combine`. `lib/intake/assemble.ts` is the transport (the sibling of `combineData`) + `looksLikeScrnaDeposit`, deliberately narrow because a wrong guess sends the user to the wrong error message. The assembled `.h5ad` re-enters the ordinary `ingest()` path, so it uploads and survives reload re-runnable. Drop-zone copy + accept list updated so it is discoverable, labelled busy state, and a result note ("Assembled 4 samples · 21,300 cells × 20,000 genes"). MSW handler added; `/data/assemble-scrna` waiver deleted. |
| L2-06 | B22 | Label the "Import" busy state (currently an unlabelled spinner). | **DONE** — the button now shows a spinner **beside** the words "Importing…", carries `aria-busy`, and keeps a stable accessible name that says which provider it is importing from. The change itself rode in with the L2-01 menu rewrite (same control, same file — writing the unlabelled spinner back just to re-delete it would have been theatre); this row's own commit adds the ratchet: `lib/cloud/import-surface.guard.test.ts` rejects the exact prior shape (`{busy ? <Loader2 …/> : "Import"}`), verified to FAIL when that shape is reinstated. A spinner *beside* text stays legal. |
...
| L3-04 | §D | Observe the review's unverified layout predictions in a real browser at desktop widths and close or re-file them honestly. | **DONE as a reporting row — nothing closed on the lane's word.** No browser can run in a worktree (`next dev` is fatal on the out-of-root symlink), so each of §D's 12 bullets became a runnable check in `LANE-WRAP.md` §D: what to load · which viewport · the console snippet · the PASS/FAIL signature. Two facts §D could not know: (1) three bullets are **out of date** — `D-4`/`D-5`/`D-11` describe chrome and sizing this lane changed; (2) the annotation layer is now flag-gated **off**, so `D-2` is unreachable in a default build and `D-1`/`D-6`/`D-7` lose the half that motivated them — a flag-on check is testing deferred Plan C territory and must be filed there. `D-1` gains a prediction §D missed: at the now-maximum **6** tabs, "Legend" (~32px) still overflows its 27.8px label box. **The lead runs the list on the rebased main checkout and closes or re-files each one there.** |
